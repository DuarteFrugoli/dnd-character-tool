...
class AddItemSheet extends ConsumerStatefulWidget {
  const AddItemSheet({super.key, required this.characterId});
...
class _AddItemSheetState extends ConsumerState<AddItemSheet>
  ...
  Future<void> _confirmAdd({
    ...
    final itemName = stripAmmunitionPackSuffix(name, itemType);
    final itemDisplayName = displayName == null
        ? null
        : stripAmmunitionPackSuffix(displayName, itemType);
    final unitWeight = inventoryUnitWeight(name, itemType, weight);
    var quantityText = '1';
  ...
  Widget _buildSrdListTile({
    ...
    final cleanDisplayName = stripAmmunitionPackSuffix(
      displayName ?? name,
      itemType,
    );
  ...
  SrdInventorySearchCatalog? _globalSrdSearchCatalog(
    ...
    _searchCatalog = SrdInventorySearchCatalog.fromSrd(
      ...
      i18n: i18n,
      l10n: l10n,
      itemTypeLabel: (type) => itemTypeLabel(type, l10n),
      categoryLabel: (category) => categoryLabel(category, l10n, i18n),
    );
      ...
      children: filtered
          .map(
            (entry) => _buildSrdListTile(
              ...
              subtitle:
                  '${itemTypeLabel(entry.itemType, l10n)}  ·  ${entry.subtitle}',
  ...
  Widget _buildGroupedSrdList<T>({
    ...
    final filtered = q.isEmpty
        ...
        : items.where((e) {
            ...
            final display = stripAmmunitionPackSuffix(
              (getDisplayName ?? getName)(e),
              itemType,
            ).toLowerCase();
            final english = stripAmmunitionPackSuffix(
              getName(e),
              itemType,
            ).toLowerCase();
